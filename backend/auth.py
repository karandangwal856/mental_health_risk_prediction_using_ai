...
async def get_current_user(token: Optional[str] = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    # Bypassing authentication for Guest mode as requested
    guest_user = db.query(models.User).filter(models.User.email == "guest@mindcheck.ai").first()
    if not guest_user:
        # Create a default guest user if it doesn't exist
        guest_user = models.User(
            email="guest@mindcheck.ai",
